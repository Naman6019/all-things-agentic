...
export function ScrollVelocityContainer({
  ...
  // Gentle, calm velocity factor that never accelerates wildly
  const velocityFactor = useTransform(smoothVelocity, (v) => {
    const sign = v < 0 ? -1 : 1;
    const magnitude = Math.min(0.8, (Math.abs(v) / 3000) * 0.5);
...
function ScrollVelocityRowImpl({
  ...
  useAnimationFrame((_, delta) => {
    ...
    const absVf = Math.min(0.8, Math.abs(vf));
    ...
    if (bw <= 0) return;
    // Calibrated smooth drift rate (20-30 px/sec)
