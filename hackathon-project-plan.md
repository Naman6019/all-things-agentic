...
### Architecture
Cloud Run (ADK conversational agent, session state) → Gemini 3.5 (drives clarifying questions, spec generation, interprets feedback deltas) → Firestore (persistent style profile + wireframe version history) → a small rendering layer (layout JSON → HTML/SVG) → simple web UI for viewing/iterating.
...
- [ ] At least one Google agent framework — ADK for both, per this plan
- [ ] At least one GCP infra service — Cloud Run + Firestore covers this with room to spare
